...
import { Chain, ClobClient, OrderType, Side, SignatureTypeV2, type ApiKeyCreds, type TickSize } from "@polymarket/clob-client-v2";
...
import { isValidJournalOrder, settlementTimedOut, type JournalOrder } from "../app/lib/live-order-journal";
import { evaluatePaperMarket, type PaperOpportunity } from "../app/lib/paper-bankroll";
import { quoteMinimumShareBuy, quoteSell, takerFeePerShare } from "../app/lib/live-order-pricing";
import { openPositions, type WalletPosition } from "../app/lib/wallet-positions";
import { loadCalibrationFile } from "./calibration-file";
import { money, number, openExposureUsd, readBalance, readClosedTrades, readPositions, reconcileJournalOrder, redemptionNotice, sleep } from "./live-account";
import { liquidationEquityUsd, rememberHeldBooks, type PositionBook } from "../app/lib/live-equity";
...
const positionBooks = new Map<string, PositionBook>();

/** Liquidation equity against held positions' executable bids (see app/lib/live-equity.ts). */
const liveEquity = (balance: number, positions: PositionRow[]) => liquidationEquityUsd({ balance, positions, books: positionBooks,
  serverNow: synchronizedPolymarketTime(), fallbackFeeRate: RISK.feeRate, maxBookAgeMs: MAX_ORDER_BOOK_AGE_MS * 3 });

const rememberPositionBooks = (positions: PositionRow[], markets: Iterable<LiveMarket>) => rememberHeldBooks(positionBooks, positions, markets);
...
/**
 * Why no new entry can be placed at this balance, or null when one can. Call
 * it only after the held positions' books are loaded (refreshPositionBooks):
 * a position without a book is valued at zero, which would understate equity.
 */
const entryEligibilityIssue = (balance: number, positions: PositionRow[]): string | null => {
  const equity = liveEquity(balance, positions);
  if (equity < LIVE_MINIMUM_EQUITY_USD) return `Liquidation equity ${money(equity)} is below the $10 live-entry floor.`;
  if (balance < 1) return "Available USDC is below the $1 minimum executable stake.";
  const feasibility = liveEntryFeasibility(equity);
  if (!feasibility.feasible) {
    return `No entry can pass at this balance: the ${feasibility.profile.tier} tier caps an entry at ${money(feasibility.perEntryCap)}, but the smallest allowed order (${LIVE_MINIMUM_SHARES} shares at the tier's ${Math.round(feasibility.profile.minEntryPrice * 100)}c floor, with fees) costs ${money(feasibility.cheapest)}. At least ${money(feasibility.requiredEquity)} of equity is needed.`;
  }
  return null;
};
...
async function main() {
  ...
  console.log("This connects to the real CLOB. The wallet address and signature type may come from the local .env file; the signer private key is only accepted at the hidden prompt below and is never written to disk. FAK orders can partially fill and cancel the remainder. Nothing here guarantees a profit; the model weight in use is reported before arming.\n");
  ...
  try {
    ...
    // Held positions are marked against their books; load them before any
    // equity figure is used, including when resuming a settling order.
    await refreshPositionBooks(positions, new Set());
    const notice = redemptionNotice(positions);
    if (notice) console.log(notice);
    if (!state.pending) {
      const startupIssue = entryEligibilityIssue(balance, positions);
      if (startupIssue) throw new Error(startupIssue);
    }
    ...
    /** Set while no new entry can be afforded; exits and settlement keep running, and it clears when equity recovers. */
    let entryPause: string | null = null;
    let lastEntryPauseLogAt = 0;
    const refreshEntryPause = (balanceNow: number, positions: PositionRow[]) => {
      const issue = entryEligibilityIssue(balanceNow, positions);
      if (issue !== entryPause) {
        console.log(`[${new Date().toLocaleTimeString()}] ${issue ? `New entries paused: ${issue} Exits and settlement continue.` : "New entries resumed: the balance can afford an entry again."}`);
        lastEntryPauseLogAt = Date.now();
      }
      entryPause = issue;
    };
    ...
    while (!controller.signal.aborted) {
      ...
      if (state.pending) {
        ...
        if (outcome === "SETTLED") {
          // The fill just changed the holdings: mark the new position against
          // its own book before judging whether another entry is affordable.
          try {
            await refreshPositionBooks(lastKnownPositions, new Set());
            refreshEntryPause(lastKnownBalance, lastKnownPositions);
          } catch (error) {
            console.log(`[${new Date().toLocaleTimeString()}] Post-settlement mark held: ${scrubError(error, privateKey)}`);
          }
        }
      ...
      try {
        ...
        if (now - lastPortfolioRefreshAt >= PORTFOLIO_REFRESH_MS) {
          ...
          refreshEntryPause(refreshedBalance, refreshedPositions);
          lastPortfolioRefreshAt = now;
          await writeState(state);
          lastStateWriteAt = now;
        }
        ...
        if (entryPause) {
          if (Date.now() - lastEntryPauseLogAt >= 60_000) {
            lastEntryPauseLogAt = Date.now();
            console.log(`[${new Date().toLocaleTimeString()}] New entries paused: ${entryPause}`);
          }
          await sleep(SCAN_MS);
          continue;
        }
