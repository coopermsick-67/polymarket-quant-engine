...
The recorder passed a one-minute smoke test, kept separate from the gate sample: 12,113 raw messages (11,950 Binance perpetual, 161 RTDS, 2 CLOB), 11,905 venue ticks, 80 snapshots from 16 market IDs, 400 decisions, 0 simulated fills, and 0 official resolutions. Binance spot, Bybit, and OKX sockets did not stay connected in that run. This verifies writes only; the smoke data is not used to claim any gate. Current empirical results below are the verified historical measurements available at this audit. `n` means the count reported by those measurements, not a count inferred from the SQLite recorder.

The recorder now writes a feed-aware heartbeat, restarts a child whose heartbeat or one-second paper loop is stale, applies bounded restart backoff, and uses a SQLite lease to reject a second supervisor on the same data directory. `pnpm run runner:status` distinguishes a healthy process from degraded or missing core feed data and reports secondary venue state. Unit coverage is in `tests/runner-health.test.ts` and `tests/venue-feeds.test.ts`; local `pnpm run check` passed 92 tests with one E2E test skipped, and `pnpm run build` completed. The systemd service and nightly timer are templates only: they have not been installed on an always-on host, and no continuous run or new market sample is claimed from this change. Local `pnpm run test:e2e` could not launch because Chromium is absent; the attempted Playwright download returned a truncated archive.
...
| G6: Live safety tests              | Kill switch, dead-man switch and daily-loss stop each tested in canary.                                                                                                                         | No canary safety tests have been run. There is no headless live runner. The browser-side order loop has been removed, and new buys and sells are rejected server-side; these fail-closed controls do not count as canary tests.           | **NOT MET**      |

**Decision: NO-GO for real money.** G1 and G2 fail on the available measurements; G3–G6 lack qualifying evidence. The data says the book is better calibrated, predicted edge materially exceeded realized edge, and the paper strategy lost money. Keep live order placement disabled. The headless runner now records daily SQLite databases with raw Polymarket/Coinbase and Binance/Bybit/OKX feed messages, normalized venue ticks, snapshots, decisions, paper fills, and official data. Continue recording and backfill with `pnpm run backfill -- --data-dir data`; run `pnpm run report -- --data-dir data` nightly for untouched expanding walk-forward folds, market-cluster confidence intervals, reliability tables, and explicit gate statuses. No claim is made that the required 7 days or 3,000 resolved markets have been collected.
