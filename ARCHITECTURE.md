...
| `scripts/headless.ts`        | Headless paper/shadow loop with atomic state persistence, a feed-aware heartbeat, SQLite recording and Telegram alerts.                                                                  |
| `scripts/headless-supervisor.ts` | Restarts crashed or stalled paper loops with bounded backoff; holds a SQLite singleton lease so two supervisors cannot write the same data directory.                                |
| `scripts/runner-health.ts`   | Heartbeat schema, freshness checks and restart backoff policy shared by the supervisor, status command and tests.                                                                        |
| `scripts/runner-lock.ts`    | Crash-recoverable SQLite lease for one supervisor per data directory.                                                                                                                     |
| `scripts/runner-status.ts`  | Checks the latest heartbeat timestamp, tick time and runner process on the current host.                                                                                                  |
| `deploy/systemd/`            | Restart-on-failure paper service and persistent nightly backfill/report timer for an always-on Linux host.                                                                                |
...
- Headless liveness: `data/runner-heartbeat.json` records process identity, tick freshness and feed connection state. The supervisor checks it every five seconds, restarts a runner after a stale heartbeat/tick, and uses `data/runner-supervisor.sqlite` to prevent duplicate supervisors. These files demonstrate local process health only; they do not prove continuous uptime or complete market coverage.
